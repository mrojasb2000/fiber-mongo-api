...
import (
	...
	"github.com/mrojasb2000/fiber-mongo-api/models"
	"github.com/mrojasb2000/fiber-mongo-api/responses"
	"go.mongodb.org/mongo-driver/bson"
	"go.mongodb.org/mongo-driver/bson/primitive"
	"go.mongodb.org/mongo-driver/mongo"
)

var userController *mongo.Collection = configs.GetCollection(configs.DB, "users")
...
func CreateUser(c *fiber.Ctx) error {
	...
	result, err := userController.InsertOne(ctx, newUser)
	if err != nil {
...
func GetAUser(c *fiber.Ctx) error {
	...
	err := userController.FindOne(ctx, bson.M{"id": objId}).Decode(&user)
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(responses.UserResponse{Status: http.StatusInternalServerError, Message: "error", Data: &fiber.Map{"data": err.Error()}})
	}
...
func EditAUser(c *fiber.Ctx) error {
	...
	result, err := userController.UpdateOne(ctx, bson.M{"id": objId}, bson.M{"$set": update})
	...
	if result.MatchedCount == 1 {
		err := userController.FindOne(ctx, bson.M{"id": objId}).Decode(&updatedUser)
...
func DeleteAUser(c *fiber.Ctx) error {
	...
	result, err := userController.DeleteOne(ctx, bson.M{"id": objId})
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(responses.UserResponse{Status: http.StatusInternalServerError, Message: "error", Data: &fiber.Map{"data": err.Error()}})
	}
...
func GetAllUsers(c *fiber.Ctx) error {
	...
	results, err := userController.Find(ctx, bson.M{})
